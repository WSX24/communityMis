...
async function showJuryHall(userSession) {
  if (!userSession||!userSession.user||!userSession.user.isJury) return;
  var page = document.querySelector(".jury-page");
  if (!page) return;
  page.innerHTML = '<div class="jury-hall"><div class="jury-hall-header"><h2>陪审大厅</h2><p class="jury-hall-desc">等待陪审团投票的纠纷，请公正判断</p></div><div class="jury-hall-list" id="jury-hall-list"><div class="ui-state ui-state--loading"><span class="ui-state__spinner"></span><span>正在加载纠纷列表</span></div></div></div>';
  try {
    var baseUrl = window.__API_BASE_URL__||"";
    var resp = await fetch(baseUrl+"/api/jury/disputes",{credentials:"include",headers:{"Accept":"application/json"}});
    ...
    var el=document.getElementById("jury-hall-list");
    if(el)el.innerHTML='<div class="ui-state ui-state--error"><strong>纠纷列表加载失败</strong><span>'+e.message+'</span><button class="ui-state__action" onclick="location.reload()">重试</button></div>';
  }
}
...
function renderJuryHallList(disputes,userSession) {
  ...
  if(!list)return;
  if(!disputes||disputes.length===0){list.innerHTML='<div class="ui-state ui-state--empty">当前没有待投票的纠纷</div>';return}
  list.innerHTML=disputes.map(function(d){return '<a href="/jury/voting?disputeId='+d.disputeId+'" class="jury-dispute-card"><div class="jury-card-header"><span class="jury-card-id">#'+d.disputeId+'</span><span class="jury-card-type">'+(d.type==="quality_issue"?"质量问题":d.type==="communication"?"沟通纠纷":d.type==="not_completed"?"未完成":"其他")+'</span></div><div class="jury-card-body">'+(d.reason||"").substring(0,100)+'</div><div class="jury-card-footer"><span>订单 #'+d.orderId+'</span><span class="jury-card-arrow">参与投票 &rarr;</span></div></a>'}).join("");
}
