...
if (guardResult.status !== "redirected") {
  ...
  updateUserNav(guardResult.session);
  showJuryLobby(guardResult.session);
}

function showJuryLobby(session) {
  if (!hasUserSession(session) || !session.user.isJury) return;
  var nav = document.querySelector(".bottom-nav");
  if (!nav || document.querySelector(".jury-nav-link")) return;
  var link = document.createElement("a");
  link.href = "/jury/voting";
  link.className = "jury-nav-link";
  link.innerHTML = '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>陪审';
  nav.insertBefore(link, nav.lastElementChild);
}

// 陪审大厅：展示可投票的纠纷列表
async function showJuryHall(userSession) {
  if (!userSession || !userSession.token || !userSession.user || !userSession.user.isJury) return;
  var page = document.querySelector(".jury-page");
  if (!page) return;
  
  page.innerHTML = '<div class="jury-hall"><div class="jury-hall-header"><h2>陪审大厅</h2><p class="jury-hall-desc">以下是等待陪审团投票的纠纷，请公正判断</p></div><div class="jury-hall-list" id="jury-hall-list"><div class="ui-state ui-state--loading"><span class="ui-state__spinner"></span><span>正在加载纠纷列表</span></div></div></div>';
  
  try {
    var resp = await fetch(window.__API_BASE_URL__ + "/api/jury/disputes", {
      headers: { "Authorization": "Bearer " + userSession.token }
    });
    if (!resp.ok) throw new Error("Failed to load disputes");
    var data = await resp.json();
    renderJuryHallList(data.disputes || [], userSession);
  } catch (e) {
    document.getElementById("jury-hall-list").innerHTML = '<div class="ui-state ui-state--error">加载失败：' + e.message + '</div>';
  }
}

function renderJuryHallList(disputes, userSession) {
  var list = document.getElementById("jury-hall-list");
  if (!list) return;
  if (disputes.length === 0) {
    list.innerHTML = '<div class="ui-state ui-state--empty">当前没有待投票的纠纷</div>';
    return;
  }
  list.innerHTML = disputes.map(function(d) {
    return '<a href="/jury/voting?disputeId=' + d.disputeId + '" class="jury-dispute-card">' +
      '<div class="jury-card-header">' +
      '<span class="jury-card-id">#' + d.disputeId + '</span>' +
      '<span class="jury-card-type">' + (d.type === "quality_issue" ? "质量问题" : d.type === "communication" ? "沟通纠纷" : d.type === "not_completed" ? "未完成" : "其他") + '</span>' +
      '</div>' +
      '<div class="jury-card-body">' + (d.reason || "纠纷详情").substring(0, 100) + '</div>' +
      '<div class="jury-card-footer">' +
      '<span>订单 #' + d.orderId + '</span>' +
      '<span class="jury-card-arrow">参与投票 &rarr;</span>' +
      '</div>' +
      '</a>';
  }).join("");
}
...
function updateUserNav(session) {
  if (!hasUserSession(session)) return;
  var link = document.querySelector('.bottom-nav a[href*=profile]');
  if (!link) return;
  var name = displayName(session.user);
  var short = name.length > 6 ? name.slice(0, 5) + ".." : name;
...
async function hydrateJuryVotingRoute(session) {
  ...
  if (!disputeId) {
    await showJuryHall(userSession);
    return;
  }
