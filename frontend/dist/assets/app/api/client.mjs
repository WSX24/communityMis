...
export function createApiClient(options = {}) {
  ...
  return {
    ...
    communityPosts: {
      ...
      update: (token, postId, payload) => request("/api/community-posts/" + encodeURIComponent(postId), { method: "PUT", token, body: payload }),
      delete: (token, postId) => request("/api/community-posts/" + encodeURIComponent(postId), { method: "DELETE", token }),
      update: (token, postId, payload) => request("/api/community-posts/" + encodeURIComponent(postId), { method: "PUT", token, body: payload }), delete: (token, postId) => request("/api/community-posts/" + encodeURIComponent(postId), { method: "DELETE", token }), like: (token, postId) => request(`/api/community-posts/${encodeURIComponent(postId)}/like`, {
        method: "POST",
        token
      }),
    ...
    requests: {
      ...
      apply: (token, requestId, message) => request(`/api/requests/${encodeURIComponent(requestId)}/apply`, {
        method: "POST",
        token,
        body: { message }
      }),
      applications: (token, requestId) => request(`/api/requests/${encodeURIComponent(requestId)}/applications`, { token }),
      approveApplication: (token, appId) => request(`/api/applications/${encodeURIComponent(appId)}/approve`, {
        method: "POST",
        token
      }),
      rejectApplication: (token, appId) => request(`/api/applications/${encodeURIComponent(appId)}/reject`, {
        method: "POST",
        token
      })
    },
